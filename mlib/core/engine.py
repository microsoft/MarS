...
class Engine:
    ...
    def _add_order_owner_info(self, symbol: str, order_id: int, agent_id: int):
        key = (symbol, order_id)
        assert key not in self.order_owner, f"key existed: {key}"
        self.order_owner[key] = agent_id

    def _del_order_owner_info(self, symbol: str, order_id: int):
        key = (symbol, order_id)
        assert key in self.order_owner
        self.order_owner.pop(key)

    def _on_agent_receive_trading_result(self, event: AgentReceiveTradingResultEvent):
    ...
    def _on_receive_agent_action(self, action: Action):
    ...
    def _get_updated_agent_observation(self, event: AgentStatesUpdateAndWakeup):
    ...
    def _on_agent_states_update_and_wakeup(self, event: AgentStatesUpdateAndWakeup):
    ...
    def _on_exchange_receive_call_auction_orders(self, event: ExchangeReceiveOrdersEvent):
    ...
    def _update_order_owner(self, accepted_orders: List[LimitOrder]):
        for order in accepted_orders:
            self._add_order_owner_info(order.symbol, order.order_id, order.agent_id)

    def _on_exchange_receive_continuous_auction_orders(self, event: ExchangeReceiveOrdersEvent):
        agent = self.agents[event.agent_id]
        accepted_orders: List[LimitOrder] = []
        rejected_orders: List[BaseOrder] = []
    ...
    def _on_exchange_receive_orders_out_of_trading_period(self, event: ExchangeReceiveOrdersEvent):
        agent = self.agents[event.agent_id]
        self.push_event(
            AgentReceiveTradingResultEvent(
                time=event.time + agent.communication_delay,
    ...
    def _on_market_close_event(self, event: MarketCloseEvent):
    ...
    def _on_call_auction_end(self, event: CallAuctionEndEvent):
    ...
    def notify_agents_on_order_executed(self, time: Timestamp, transaction: Transaction):
    ...
    def _on_market_open_event(self, event: MarketOpenEvent):
